...
class Classifier(nn.Module):
	def __init__(self):
		...
		self.L3 = nn.Linear(64,17)

	def forward(self,x):
		x = F.relu(self.L1(x))
		x = F.relu(self.L2(x))
		x = F.log_softmax(self.L3(x))
		#x = F.log_softmax(self.L1(x))
		return x
...
#classification.load_state_dict(torch.load('/home/siplab/Saket/fashion/classification-206.pkl'))
...
trainset=torchvision.datasets.ImageFolder("/home/siplab/fashion/train", transform=transform, target_transform=None)
trainloader = torch.utils.data.DataLoader(trainset, shuffle = True , batch_size = batch_size , num_workers = 2)
testset=torchvision.datasets.ImageFolder("/home/siplab/fashion/valid", transform=transform, target_transform=None)
...
for epoch in range(iterations):
	...
	for i,data in enumerate(trainloader):
		...
		decoder_path = os.path.join('/home/siplab/Saket/fashion/Decoder/', 'decoder-mvc%d.pkl' %(epoch+1))
		encoder_path = os.path.join('/home/siplab/Saket/fashion/Encoder/', 'encoder-mvc%d.pkl' %(epoch+1))
		autoencoder_path = os.path.join('/home/siplab/Saket/fashion/Autoencoder/', 'autoencoder-mvc%d.pkl' %(epoch+1))
		classifier_path = os.path.join('/home/siplab/Saket/fashion/Classifier/', 'classifier-mvc%d.pkl' %(epoch+1))
		classification_path = os.path.join('/home/siplab/Saket/fashion/Classification','classification-mvc%d.pkl' %(epoch+1))
	...
	if ( epoch+1 )% 1 == 0:
		...
		with open("train_a_loss.pkl","wb") as f:
			pickle.dump(list_a_loss,f)
		with open("train_c_loss.pkl","wb") as f1:
			pickle.dump(list_a_loss,f1)
		fig = plt.figure(1)
		if epoch == 0:						
		...
		fig.savefig('/home/siplab/Saket/fashion/mvc_loss_plot.png') 
		...
		for t_i,t_data in enumerate(testloader):
			...
			at_loss = autoencoder_criterion(t_outputs , t_inputs)
			c_pred = classification(t_inputs)
			ct_loss = classification_criterion(c_pred , t_labels)
			_, predicted = torch.max(c_pred.data, 1)
			#print predicted.type() , t_labels.type()
			total += t_labels.data.size(0)
			correct += (predicted == t_labels.data).sum()
			print("saving image")
			test_result_path = os.path.join('/home/siplab/Saket/fashion/Test_results/', 'batch_%d_%d_MVC'%((epoch+1)/1,t_i+1) + '.jpg')
			image_tensor = torch.cat((t_inputs.data[0:8], t_outputs.data[0:8]), 0)
			torchvision.utils.save_image(image_tensor, test_result_path)
			run_at_loss += at_loss.data[0]
			run_ct_loss += ct_loss.data[0]
		print('Accuracy of the network on the 8000 test images: %d %%' % (100 * correct / total))

		if (epoch + 1)%1 == 0:
			list_at_loss.append(run_at_loss/1000)
			list_ct_loss.append(run_ct_loss/1000)
			with open("test_a_loss.pkl","wb") as f2:
				pickle.dump(list_at_loss,f2)
			with open("test_c_loss.pkl","wb") as f3:
				pickle.dump(list_ct_loss,f3)
			plt.figure(2)
			if epoch == 0:						
				plt.legend(loc='upper left')
				plt.xlabel('Epochs')
				plt.ylabel('Loss')
			plt.plot(range(epoch+1),list_at_loss,'r--',label='autoencoder')
			plt.plot(range(epoch+1),list_ct_loss,'b--',label='classifier')
			fig.savefig('/home/siplab/Saket/fashion/mvc_v_loss_plot.png')
