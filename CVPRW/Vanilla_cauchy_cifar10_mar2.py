...
for epoch in range(start,iterations):
    ...
    for i,data in enumerate(trainloader,0):
        ...
        try:
            loss = loss_criterion(torch.squeeze(cauchy_output),labels.float())
        except RuntimeError: 
            print("s",torch.max(labels.float()).item(),torch.min(labels.float()).item())
        ...
        loss_count = 50 #after how many iterations do you want to show loss
        if (i +1) % loss_count == 0:
    ...
    data_store_path = '/home/saket/CVPRW/log_vanilla_cauchy_cifar'
    ...
    if ( epoch+1 )% 1 == 0:
        with torch.no_grad():
            ...
            for t_i,t_data in enumerate(testloader):
                ...
                correct += len(new_cauchy_output)- (new_cauchy_output ^ t_labels.byte()).sum()
                
                try:
                    t_loss += loss_criterion(torch.squeeze(cauchy_output),t_labels.float()).item()
                except RuntimeError:
                    print("s ",torch.max(t_labels.float()).item(),torch.min(t_labels.float()).item())
                    print("\nCO ",torch.max(torch.squeeze(cauchy_output)).item(),torch.min(torch.squeeze(cauchy_output)).item())
