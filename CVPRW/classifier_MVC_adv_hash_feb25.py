...
disc1_criterion = nn.NLLLoss()
disc2_criterion = nn.NLLLoss()
...
for epoch in range(start,iterations):
    ...
    for i,data in enumerate(trainloader,0):
        ...
        if i%2==0 and not len(indexes)==batch_size:
            ...
            d_loss = disc1_criterion(torch.squeeze(d_out),f_labels.long())
        ...
        if i%2==1:
            ...
            d_out = disc2(d_input)
            # print(d_out)
            d_loss = disc2_criterion(torch.squeeze(d_out),s.long())
            # print(d_loss)
        ...
        # h1 = encoder(x)
        # h2 = encoder(y)
    data_store_path = '/home/saket/CVPRW/log_classifier'
        ...
        with torch.no_grad():
            ...
            for t_i,t_data in enumerate(testloader):
                ...
                if not len(indexes)==batch_size:
                    ...
                    d_out = torch.squeeze(disc1(d_input))
                    # print("d_out",d_out)                
                    _,d1_pred =  torch.max(d_out.data , 1)
                    # print("d1_pred",d1_pred)
                    # print("d1_label",d1_labels)
                    d1_total += len(d1_labels)
                    d1_correct += (d1_pred == d1_labels).sum().cpu().numpy()
                ...
                _,d2_pred =  torch.max(d2_out.data , 1)
                # print("d2_pred",d2_pred)
                # print("d2_label",d2_labels)
                d2_total += len(d2_labels)
                # print("d2_total",d2_total)
                d2_correct += (d2_pred == d2_labels.long()).sum().cpu().numpy()
                # torch.squeeze(d_out),f_labels.float()
